---
itle: "Breaking Down a Monolith"
subtitile: "An Analysis of Asian Housing Outcomes in the United States"
author: "Tate Mason"
date: "2024-5-28"
output:
  html_document:
...
First, we will need to take some care to format the data appropriately.
...
for (r in races) {
  for (y in years){
    ...
    p2 <- ggplot(ipums_data[ipums_data$year == y & ipums_data$renter == 0, ], aes(x=inctot_trimmed, y=hval_inc)) +
      geom_bin2d(bins=nq) +
      facet_wrap(as.formula(paste("~", r))) +
      labs(title=paste0(y, "House Value to Income Ratio"), x='Income', y='Value-to-Income (%)') +
      theme_minimal()
    ggsave(file.path(output_path, paste0(r, "_extend_valsum", y, ".pdf")), p2, device='pdf')
    p3 <- ggplot(ipums_data[ipums_data$year==y, ], aes(x=inctot_trimmed, y=own100)) +
      geom_bin2d(bins=nq) +
      facet_wrap(as.formula(paste('~', r))) +
      labs(title=paste0(y, ' Ownership Rate'), x='Income', y='Ownership Rate') +
      theme_minimal()
    ggsave(file.path(output_path, paste0(r, '_extend_ownsumm', y, '.pdf')), p3, device='pdf')
  }
}
```

Here is the plot of rent to income ratio
```{r}
p1
```

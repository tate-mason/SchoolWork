...
for r in eachrow(profiles)
    ...
    if haskey(state_to_i, s) && haskey(age_to_t, a)
        i = state_to_i[s]
        t = age_to_t[a]
        wage[i,t] = Float64(r.wage)
        tax[i,t]  = Float64(r.tax_rate)
        rent[i,t] = Float64(r.rentgrs)
...
try
    ...
    @warn "Failed to load env_state_amenities.csv; using zeros for amenR_base" err
    amenR_base .= 0.0
end
...
env = MigCalibration.DataEnv(
  Z, ages, β, σ, prob_retire, prob_death, pension_share,
  amenW_base, amenR_base,
  wage, tax, rent, hccost,
  init_dist_40,
...
targets_agepair_df = CSV.read("targets_agepair_probs.csv", DataFrame)
ages_data = targets_agepair_df.age_start
p_data    = targets_agepair_df.p

# ----------- Run Calibration -----------
θ_0 = (5.0, 5.0, 0.0, 0.0) # initial guess of disutilities and amenities
θ_hat, res = MigCalibration.run_calibration(
  θ_0, targets, env;
  n_agents = n_agents)
println("Estimated parameters: ", θ_hat)

moms = MigCalibration.simulate_model_moments(tuple(θ_hat...), env; n_agents=100_000)

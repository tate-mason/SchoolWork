...
\begin{frame}{Value Functions, cont.}
  Value of returning to an occupation j':
  \begin{gather*}
    M_s(\Omega_s, z, j') = \int V_s(\Omega_s, z, \epsilon, j')dF_{j'}(\epsilon)
  \end{gather*}
\end{frame}
...
\begin{frame}{Estimation}
  \begin{itemize}
    ...
    \item panel earnings: $y_{it} = \alpha_i + \eta\cdot H_{it} + v_{it}$
    \item probability of switching: $P_{it} \equiv Pr(y_{it}=1|\bar{\hat{v_{it-1}}}) = \psi(\bar{\hat{v_{it-1}}};\beta)$
    \item mobility cost function: $c(s, \kappa) = \kappa_0 + \kappa_{1s} + \kappa_{2s}^2$ 
  \end{itemize}
\end{frame}

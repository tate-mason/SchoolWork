...
\begin{frame}{Model Overview}
  \begin{itemize}
    ...
    \item Workers supply labor inelastically, earn wages based on location
    \item Retirees receive pension income, base moving decisions on amenities
...
\begin{frame}{Decision Problem: Workers}
  ...
  \end{itemize}
  \begin{equation}
    \item $V^S_w(s_t, z_t; t) = \max_{c_t, s_{t+1}} \{U(c_t, m(z_t), \mu) + x\beta \sum_{\mu'}\Pi(\mu'|\mu)V^w(s_{t+1}, z_t; t+1)\}$
  \end{equation}
  \begin{equation}
    \item $V^M_w(s_t, z_t; t) = \max_{z', c_t, s_{t+1}} \{U(c_t, m(z'), \mu) + x\beta \sum_{\mu'}\Pi(\mu'|\mu)V^w(s_{t+1}, z'; t+1)\}$
  \end{equation}
\end{frame}
\begin{frame}{Decision Problem, Retirees}
  ...
  \end{itemize}
  \begin{equation}
    \item $V^S_r(s_t, z_t; t) = \max_{c_t, s_{t+1}} \{U(c_t, m(z_t)) + \beta V^r(s_{t+1}, z_t; t+1)\}$
  \end{equation}
  \begin{equation}
    \item $V^M_r(s_t, z_t; t) = \max_{z', c_t, s_{t+1}} \{U(c_t, m(z')) + \beta V^r(s_{t+1}, z'; t+1)\}$
  \end{equation}
\end{frame}
\section{Next Steps}

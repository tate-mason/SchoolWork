...
$D_i$ by formation cannot deviate from compliance. When $Z_i = 0$, The sum of $\alpha_0, \alpha_U, \epsilon_i^D$
...
df <- df %>%
  mutate(
    ...
    D_i = as.numeric(-4 + 5 * Z_i + 4 * U_i + epsilon_D > 0),
    Y_i = 3 + 2 * D_i + 0 * Z_i + 6 * U_i + epsilon_Y,
    pNT = sum(Z_i == 1 & D_i == 0) / n,
    pAT = sum(Z_i == 0 & D_i == 0) / n,
    pC = 1 - pAT - pNT
  )
df$pC
df$pNT
...
The proportion remains the same as the coefficient on $Z_i$ does not effect anything when $Z_i = 0$. Thus, all
units will comply.

## Part 10: How does correlation between $D \text{and} Z$ vary with the proportion of compliers when we change $\alpha_Z$?
While the proportion of compliers does not vary, we see higher correlation when $\alpha_Z = 10$, such that $corr(Z_i, D_i) = 0.96$.
When $\alpha_Z = 5$, $corr(Z_i, D_i) = 0.67$. 

## Part 11: Show that we can justify the claim of no external validity problems from item 1.

...
stage1 <- lm(p_jmt ~ sugar_price + fiber + sugars + flavored + fortified +
                 market_fe + manufacturer_fe + time_fe + brand_fe,
               data = reg_data)
summary(stage1)
reg_data <- reg_data %>%
...
for (mkt in unique(reg_data$market_id)) {
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      if (i == j) {
        elas_matrix[i, j] <- coef(stage2)["p_hat"] * mkt_data$p_jmt[i] * (1 - mkt_data$sj[i])
      } else {
        elas_matrix[i, j] <- -coef(stage2)["p_hat"] * mkt_data$p_jmt[j] * mkt_data$sj[i]
      }
    }
  }
...
reg_data <- reg_data %>%
  mutate(
    own_price_elasticity_2sls = coef(stage2)["p_hat"] * p_jmt * (1 - sj),
...
s1_nest <- lm(p_jmt ~ sugar_price + z_cond + fiber + sugars + flavored + fortified +
                 market_fe + manufacturer_fe + time_fe + brand_fe,
               data = reg_data)
summary(s1_nest)
reg_data <- reg_data %>%
  mutate(p_hat_nest = predict(s1_nest))
s2_nest <- lm(u ~ p_hat_nest + fiber + sugars + flavored + fortified + share_con +
                 market_fe + manufacturer_fe + time_fe + brand_fe,
               data = reg_data)
summary(s2_nest)
# Histogram of own price elasticities (2SLS Nested)
reg_data <- reg_data %>%
  mutate(own_price_elasticity_2sls_nest = coef(s2_nest)["p_hat_nest"] * p_jmt * (1 - sj))
...
for (mkt in unique(reg_data$market_id)) {
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      if (i == j) {
        elas_matrix_nest[i, j] <- coef(s2_nest)["p_hat_nest"] * mkt_data$p_jmt[i] * (1 - mkt_data$sj[i])
      } else if (mkt_data$nest[i] == mkt_data$nest[j]) {
        elas_matrix_nest[i, j] <- -coef(s2_nest)["p_hat_nest"] * mkt_data$p_jmt[j] * (mkt_data$sj[i] * (1 - mkt_data$share_con[i]))
      } else {
        elas_matrix_nest[i, j] <- -coef(s2_nest)["p_hat_nest"] * mkt_data$p_jmt[j] * (mkt_data$sj[i] * (1 - mkt_data$share_con[i]))
      }
    }
...
reg_data <- reg_data %>%
  mutate(
    ope_nest = coef(s2_nest)["p_hat_nest"] * p_jmt * (1 - sj),

...
for (mkt in unique(reg_data$market_id)) {
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      if (i == j) {
        elas_matrix_nest[i, j] <- coef(s2_nest)["fit_p_jmt"] * mkt_data$p_jmt[i] * (1 - mkt_data$sj[i])
      } else if (mkt_data$nest[i] == mkt_data$nest[j]) {
        elas_matrix_nest[i, j] <- -coef(s2_nest)["fit_p_jmt"] * mkt_data$p_jmt[j] * (mkt_data$sj[i] * (1 - mkt_data$share_con[i]))
      } else {
        elas_matrix_nest[i, j] <- -coef(s2_nest)["fit_p_jmt"] * mkt_data$p_jmt[j] * (mkt_data$sj[i] * (1 - mkt_data$share_con[i]))
      }
    }

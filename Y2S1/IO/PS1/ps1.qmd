...
for (mkt in unique(reg_data$market_id)) {
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      if (i == j) {
        elas_matrix_nest[i, j] <- coef(s2_nest)["fit_p_jmt"] * mkt_data$p_jmt[i] *(1/(1-coef(s2_nest)["fit_sjg"]))*((1-coef(s2_nest)["fit_sjg"]*mkt_data$sjg[i]) - (1-coef(s2_nest)["fit_sjg"])*mkt_data$sj[i])

...
iri <- read_tsv(here("IRI.csv"), show_col_types = FALSE) %>%
  mutate(
    market_id = interaction(store_id, week_id, drop = TRUE),
    time_id = week_id
  )
...
iri <- iri %>%
  ...
  mutate(
    ...
    s0  = pmax(1 - sum(quantity, 0)) / first(M), .Machine$double.eps)
  ) %>%
  ungroup()

inc <- read_tsv(here("simulated_agents_income.csv"), show_col_types = FALSE)
kids <- read_tsv(here("simulated_agents_nchild.csv"), show_col_types = FALSE)
...
agent_draws <- long_inc %>%
  inner_join(long_kids, by = "draw_id") %>%
  mutate(agent_id = row_number()) %>%
  select(agent_id, draw_id, income, nchild)

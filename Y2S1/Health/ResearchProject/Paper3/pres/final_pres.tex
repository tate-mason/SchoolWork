...
\section{Empirical Strategy}
  ...
  \begin{frame}{Model - Addicts}
    \begin{itemize}
      ...
      \item Each period, addict $a_i$ will search for a physician to get drugs, which they will be prescribed with probability $a(\theta)$, where $\theta$ is the measure of physicians willing to prescribe
      \item Key variables include:
      \begin{itemize}
        \item $s$: search cost
        \item $r_i$: risk tolerance of addic $i$
  ...
  \begin{frame}{Addict's Problem}
    In the first period:
    \begin{gather*}
      V_a^0(\gamma_i, r_i) = u(c_i) + \beta(\mathbb{E}(1-a(\theta))V_a(\gamma_i, r_i) + \mathbb{E}a(\theta)V_a(\gamma, r')) \\
      \text{s.t. } \\

...
\begin{gather*}
  ...
  V_a^f(\gamma_i, r_i') = \{u(c_i) + \beta(1-a(\theta))V_a(\gamma_i, r_i') + \beta(a(\theta)m(\theta))V_a(\gamma_i, r_i'') + \beta(a(\theta)(1-m(\theta)))V_a(\gamma_i, r_i'')\} \\
  u(c) = \frac{c_i^{1-\sigma}}{1-\sigma} \\
  r_i' = r_i + \epsilon_a
\end{gather*}

Further, both risk, $r_i$, and tolerance $\gamma_i$ are drawn from normal distributions, and $a(\theta), \ m(\theta)$ are drawn from uniform distrubtions.

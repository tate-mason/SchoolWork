...
\begin{gather*}
  ...
  u(c) = \frac{c_i^{1-\sigma}}{1-\sigma} \\
  r_i' = r_i + \epsilon_a \\
  c_i =\gamma_ip_{it} + r_i^{1/\gamma_i} - \sum s_{it}
\end{gather*}
...
to search again, $V_a^f$, or to forgo search, $V_a^s$. $u(c_i)$ is the utility from consumption, with CRRA parameter $\sigma$. Risk evolves as before, and 
consumption is a function of risk tolerance and type, with higher type addicts consuming more for a given risk level due to their predisposition. Addicts
will continue to search until they are cut off, which happens when their risk exceeds a threshold set by physicians. Finally, the constraint on consumption
is made up of prescription amount, $p_{it}$, risk tolerance, and search cost.
...
\begin{gather*}
  \shortintertext{s.t.} \\
  v_j = r_{jt}^2\sum\limits_{i=1}^{I} c_{it} + \xi \\
\end{gather*}
...
for a visit. This can be thought of as the co-pay for a visit. $i_j$ is a flat physician's insurance cost which is raised by amount of risk. This can be thought of as
malpractice insurance, or something to that effect. Basically, they want to maximize visit revenue, net of their risk adjusted fixed cost.

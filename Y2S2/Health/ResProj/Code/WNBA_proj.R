...
run_cs <- function(yname, sport_label, data = df,
  ...
  es_df <- tibble(
    ...
    att    = agg_dyn$att.egt,
    se     = agg_dyn$se.egt,
    crit   = agg_dyn$crit.val.egt # uniform CB critical value
  ) %>%
    mutate(
      att_norm = att - att[e == -1], # re-center on t-1 estimate
...
make_spaghetti <- function(sport_col, sport_label) {
  ...
  ggplot() +
    ...
    # WNBA founding year
    geom_vline(
      xintercept = 1997, linetype = "dotted",
      color = "grey40", linewidth = 0.6
    ) +
    annotate("text",
      x = 1997.3, y = Inf, label = "WNBA founded",
...
make_cohort_trend <- function(sport_col, sport_label) {
  ...
  ggplot(
    cohort_means,
    aes(
      x = year, y = mean_idx, color = cohort_label,
      linewidth = I(lwd), alpha = I(alpha_val)
    )
  ) +
    geom_line() +
    geom_point(
      data = filter(cohort_means, is_never),
    ...
    labs(
      ...
      subtitle = paste0(
        "Each coloured line = mean across states in that cohort",
...
cohort_years <- sort(unique(team_state$first_treat))
cohort_fills <- setNames(
  brewer.pal(length(cohort_years), "Set1"),

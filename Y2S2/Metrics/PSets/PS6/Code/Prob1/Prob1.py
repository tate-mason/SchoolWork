...
def log_lik(theta):
    ...
    delta = theta[6]
    beta = theta[7]
    eps = np.random.gumbel(loc=0, scale=1, size=(N, T, J))
    ...
    for t in range(T-1, -1, -1):
        ...
        for j in range(1, J):
            u[:,j] = (beta_s[j-1] + beta_d[j-1]*X_d[:, t] - delta*(prev_choice != j))
...
res_df = pd.DataFrame({
    "Parameter": ["beta_s1", "beta_s2", "beta_s3", "beta_d1", "beta_d2", "beta_d3", "delta", "beta"],
    "Estimate": res.x
})
print(res_df)

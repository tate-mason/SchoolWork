...
\begin{frame}{Pollak (1970): Habit Formation}
  \begin{tcolorbox}[colback=red!5!white, colframe=UGARed, title=\textbf{Citation}]
    ...
    \textit{Journal of Political Economy}, 78(4), 745--763.
  \end{tcolorbox}
  \vspace{6pt}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \hl{What they do:}
      \begin{itemize}
        \item Model consumer preferences as influenced by past consumption
        \item Show conditions under which consumers derive utility from previously consumed goods
        \item Theoretical foundation for state-dependence in demand
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \hl{Relevance:} Seminal theoretical grounding for the idea that purchase history
      shapes current period choices --- the core mechanism of this project
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Dixit \& Stiglitz (1977): Love of Variety}
  ...
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.42\textwidth}
      \hl{Relevance:} The canonical ``love of variety'' framework --- provides the theoretical
      counterpart to Pollak's habit formation, motivating a model that nests both
    \end{column}
  \end{columns}
\end{frame}
